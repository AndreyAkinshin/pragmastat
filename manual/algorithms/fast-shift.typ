...
For samples of size $n$ and $m$, the total number of pairwise differences is $n times m$.
A naive approach would materialize all differences, sort them, and extract the desired quantile.
With $n = m = 10000$, this approach creates 100 million values,
...
After sorting both samples to obtain $x_1 <= x_2 <= ... <= x_n$ and $y_1 <= y_2 <= ... <= y_m$,
...
It maintains a search interval $["searchMin", "searchMax"]$, initialized to the range
...
For a given threshold $t$, the number of pairs $(i,j)$ satisfying $x_i - y_j <= t$ is counted.
This is equivalent to counting pairs where $y_j >= x_i - t$.
