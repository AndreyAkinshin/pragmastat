...
+ *Delegate to FastShift* — Use the #link(<sec-fast-shift>)[Fast Shift] algorithm to compute
  the desired quantile of pairwise differences in log-space.
  This leverages the $O((m + n) log L)$ complexity of FastShift.
...
This algorithm naturally extends to computing bounds on ratios:
$RatioBounds$ uses the same log-exp transformation,
delegating to $ShiftBounds$ in log-space.
