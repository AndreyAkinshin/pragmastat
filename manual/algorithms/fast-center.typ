...
A direct implementation would generate all $n(n+1)/2$ pairwise averages and sort them.
With $n = 10000$, this approach creates approximately 50 million values, requiring quadratic memory and $O(n^2 log n)$ time.
...
After sorting the values $x_1 <= x_2 <= ... <= x_n$,
  the algorithm considers the implicit upper triangular matrix $M$ where $M_(i,j) = x_i + x_j$ for $i <= j$.
...
The median corresponds to rank $k = floor((N+1)/2)$ where $N = n(n+1)/2$.
...
#source-include("cs/Pragmastat/Algorithms/FastCenter.cs", "cs")
