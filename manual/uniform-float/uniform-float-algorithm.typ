...
The generator maintains a 256-bit state ($s_0, s_1, s_2, s_3$) and produces 64-bit outputs.
...
$ r.UniformFloat() = ("next()" >> 11) times 2^(-53) $
...
$UniformInt$ maps a uniform 64-bit value into $[a, b)$ using modulo reduction.
Ranges that do not divide $2^(64)$ introduce a slight bias (acceptable for simulation,
...
All distribution samplers consume one or more $UniformFloat$ draws.
The specific transforms are documented on the corresponding distribution pages.
