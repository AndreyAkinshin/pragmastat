...
it wherever an Edgeworth expansion happens to look and nowhere else.
...
Never edit a generated fixture by hand. Change the generator and rerun it: an edited fixture
still passes every suite, because the suites read the fixture and the fixture agrees with itself,
so the disagreement with the generator is invisible until somebody regenerates. One lived here
for several commits describing a different threshold than both its filename and its generator.
Two guards now compare each generator against its output on every build, `tests:check:generator-drift`
and `tests:check:rng-fixtures`.
