test_that("avg_spread_bounds satisfies reference tests", {
  repo_root <- find_repo_root()
  test_data_dir <- file.path(repo_root, "tests", "avg-spread-bounds")
  if (!dir.exists(test_data_dir)) {
    skip("avg-spread-bounds test data directory not found")
  }
  ...
  for (json_file in json_files) {
    ...
    if (!is.null(test_case$expected_error)) {
      err <- expect_error(
        pragmastat:::avg_spread_bounds(input_x, input_y, misrate, seed = seed),
    ...
    actual_output <- pragmastat:::avg_spread_bounds(input_x, input_y, misrate, seed = seed)
