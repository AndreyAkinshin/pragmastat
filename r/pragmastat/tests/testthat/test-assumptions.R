...
call_assumption_function <- function(func_name, inputs) {
  x <- parse_array(inputs$x)
  y <- parse_array(inputs$y)
  misrate <- if (!is.null(inputs$misrate)) parse_value(inputs$misrate) else NULL
  n <- inputs$n
  seed <- inputs$seed
  ...
  switch(func_name,
    ...
    "CenterBoundsApprox" = center_bounds_approx(x, misrate, seed),
