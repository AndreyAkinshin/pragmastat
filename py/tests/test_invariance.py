...
from pragmastat import Rng, center, disparity, ratio, rel_spread, shift, spread
...
class TestInvariance:
    ...
    def test_shift_shift(self):
        self.perform_test_two(lambda x, y: shift(x + 3, y + 2), lambda x, y: shift(x, y) + 1)

    def test_shift_scale(self):
        self.perform_test_two(lambda x, y: shift(2 * x, 2 * y), lambda x, y: 2 * shift(x, y))
    ...
    def test_ratio_scale(self):
        self.perform_test_two(lambda x, y: ratio(2 * x, 3 * y), lambda x, y: (2.0 / 3) * ratio(x, y))
    ...
    def test_avg_spread_symmetry(self):
        self.perform_test_two(lambda x, y: avg_spread(x, y), lambda x, y: avg_spread(y, x))
    ...
    def test_avg_spread_scale(self):
        self.perform_test_two(lambda x, y: avg_spread(-2 * x, -2 * y), lambda x, y: 2 * avg_spread(x, y))

    # Disparity tests
    def test_disparity_shift(self):
        self.perform_test_two(lambda x, y: disparity(x + 2, y + 2), lambda x, y: disparity(x, y))

    def test_disparity_scale(self):
        self.perform_test_two(lambda x, y: disparity(2 * x, 2 * y), lambda x, y: disparity(x, y))

    def test_disparity_scale_neg(self):
        self.perform_test_two(lambda x, y: disparity(-2 * x, -2 * y), lambda x, y: -1 * disparity(x, y))

    def test_disparity_antisymmetry(self):
        self.perform_test_two(lambda x, y: disparity(x, y), lambda x, y: -1 * disparity(y, x))
