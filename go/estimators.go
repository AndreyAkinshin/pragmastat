...
const (
	centerBoundsApproxIterations   = 10000
	centerBoundsApproxMaxSubsample = 5000
	centerBoundsApproxDefaultSeed  = "center-bounds-approx"
)

// CenterBoundsApprox provides bootstrap-based nominal bounds for Center.
...
func CenterBoundsApprox[T Number](x []T, misrate float64) (Bounds, error) {
	return CenterBoundsApproxWithSeed(x, misrate, nil)
}

// CenterBoundsApproxWithSeed provides bootstrap-based nominal bounds for Center with optional seed.
func CenterBoundsApproxWithSeed[T Number](x []T, misrate float64, seed *string) (Bounds, error) {
	if err := checkValidity(x, SubjectX); err != nil {
		return Bounds{}, err
	}
	...
	minMisrate := math.Max(2.0/float64(centerBoundsApproxIterations), oneSampleMinMisrate)
	if misrate < minMisrate {
		return Bounds{}, NewDomainError(SubjectMisrate)
	}
	...
	if seed != nil {
		...
		rng = NewRngFromString(centerBoundsApproxDefaultSeed)
	}

	// m-out-of-n subsampling: cap at MaxSubsampleSize for performance
	m := n
	if m > centerBoundsApproxMaxSubsample {
		m = centerBoundsApproxMaxSubsample
	}

	centers := make([]float64, centerBoundsApproxIterations)
	for i := 0; i < centerBoundsApproxIterations; i++ {
		resample := Resample(rng, sorted, m)
		c, err := fastCenter(resample)
		if err != nil {
			return Bounds{}, err
		}
		centers[i] = c
	...
	alpha := misrate / 2
	loIdx := int(math.Floor(alpha * float64(centerBoundsApproxIterations)))
	hiIdx := int(math.Ceil((1-alpha)*float64(centerBoundsApproxIterations))) - 1
	if loIdx < 0 {
		loIdx = 0
	}
	if hiIdx >= centerBoundsApproxIterations {
		hiIdx = centerBoundsApproxIterations - 1
	}
	if loIdx > hiIdx {
		loIdx = hiIdx
	}

	bootstrapLo := centers[loIdx]
	bootstrapHi := centers[hiIdx]

	// Scale bounds to full n using asymptotic sqrt(n) rate
	if m < n {
		centerVal, err := fastCenter(sorted)
		if err != nil {
			return Bounds{}, err
		}
		scaleFactor := math.Sqrt(float64(m) / float64(n))
		lo := centerVal + (bootstrapLo-centerVal)/scaleFactor
		hi := centerVal + (bootstrapHi-centerVal)/scaleFactor
		return Bounds{Lower: lo, Upper: hi}, nil
	}

	return Bounds{Lower: bootstrapLo, Upper: bootstrapHi}, nil
}
