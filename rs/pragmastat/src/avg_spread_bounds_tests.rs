...
#[derive(Debug, Deserialize)]
struct AvgSpreadBoundsTestCase {
    input: AvgSpreadBoundsInput,
...
#[test]
fn test_avg_spread_bounds_reference() {
    let repo_root = find_repo_root();
    let test_data_dir = repo_root.join("tests").join("avg-spread-bounds");
    ...
    for json_file in &json_files {
        ...
        let test_case: AvgSpreadBoundsTestCase = serde_json::from_str(&content).unwrap();
        ...
        if let Some(ref expected_error) = test_case.expected_error {
            let result = match seed {
                Some(s) => avg_spread_bounds_with_seed(
                    &test_case.input.x,
                    &test_case.input.y,
