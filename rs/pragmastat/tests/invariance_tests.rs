...
mod invariance_tests {
    ...
    fn center_shift() {
        perform_test_one(
            |x| raw::center(&vec_add_scalar(x, 2.0), false).unwrap(),
            |x| raw::center(x, false).unwrap() + 2.0,
        );
    }
    ...
    fn center_scale() {
        perform_test_one(
            |x| raw::center(&vec_mul_scalar(x, 2.0), false).unwrap(),
            |x| 2.0 * raw::center(x, false).unwrap(),
        );
    }
    ...
    fn center_negate() {
        perform_test_one(
            |x| raw::center(&vec_mul_scalar(x, -1.0), false).unwrap(),
            |x| -1.0 * raw::center(x, false).unwrap(),
        );
    }
    ...
    fn spread_shift() {
        perform_test_one(
            |x| raw::spread(&vec_add_scalar(x, 2.0), false).unwrap(),
            |x| raw::spread(x, false).unwrap(),
        );
    }
    ...
    fn spread_scale() {
        perform_test_one(
            |x| raw::spread(&vec_mul_scalar(x, 2.0), false).unwrap(),
            |x| 2.0 * raw::spread(x, false).unwrap(),
        );
    }
    ...
    fn spread_negate() {
        perform_test_one(
            |x| raw::spread(&vec_mul_scalar(x, -1.0), false).unwrap(),
            |x| raw::spread(x, false).unwrap(),
        );
    }
    ...
    fn shift_shift() {
        perform_test_two(
            |x, y| raw::shift(&vec_add_scalar(x, 3.0), &vec_add_scalar(y, 2.0), false).unwrap(),
            |x, y| raw::shift(x, y, false).unwrap() + 1.0,
        );
    }
    ...
    fn shift_scale() {
        perform_test_two(
            |x, y| raw::shift(&vec_mul_scalar(x, 2.0), &vec_mul_scalar(y, 2.0), false).unwrap(),
            |x, y| 2.0 * raw::shift(x, y, false).unwrap(),
        );
    }
    ...
    fn shift_antisymmetry() {
        perform_test_two(
            |x, y| raw::shift(x, y, false).unwrap(),
            |x, y| -1.0 * raw::shift(y, x, false).unwrap(),
        );
    }
    ...
    fn ratio_scale() {
        perform_test_two(
            |x, y| raw::ratio(&vec_mul_scalar(x, 2.0), &vec_mul_scalar(y, 3.0), false).unwrap(),
            |x, y| (2.0 / 3.0) * raw::ratio(x, y, false).unwrap(),
        );
    }
    ...
    fn disparity_shift() {
        perform_test_two(
            |x, y| raw::disparity(&vec_add_scalar(x, 2.0), &vec_add_scalar(y, 2.0), false).unwrap(),
            |x, y| raw::disparity(x, y, false).unwrap(),
        );
    }
    ...
    fn disparity_scale() {
        perform_test_two(
            |x, y| raw::disparity(&vec_mul_scalar(x, 2.0), &vec_mul_scalar(y, 2.0), false).unwrap(),
            |x, y| raw::disparity(x, y, false).unwrap(),
        );
    }
    ...
    fn disparity_scale_neg() {
        perform_test_two(
            |x, y| {
                raw::disparity(&vec_mul_scalar(x, -2.0), &vec_mul_scalar(y, -2.0), false).unwrap()
            },
            |x, y| -1.0 * raw::disparity(x, y, false).unwrap(),
        );
    }
    ...
    fn disparity_antisymmetry() {
        perform_test_two(
            |x, y| raw::disparity(x, y, false).unwrap(),
            |x, y| -1.0 * raw::disparity(y, x, false).unwrap(),
        );
    }
}
