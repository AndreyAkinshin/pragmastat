...
public static class AvgSpreadBoundsTestCases
{
  public static void Generate()
  {
    const string suiteName = "avg-spread-bounds";
    AnsiConsole.MarkupLine($"[yellow]→[/] Generating tests for: [bold]{suiteName}[/]");

    var inputBuilder = new AvgSpreadBoundsInputBuilder();
    ...
    // Natural sequences (misrate must satisfy alpha >= min for both samples)
    ...
    // Edge cases
    inputBuilder.Add("edge-small", new Sample(1, 2, 3, 4), new Sample(2, 3, 4, 5), 1.0);
    inputBuilder.Add("edge-duplicates-mixed", new Sample(1, 1, 1, 2, 3, 4), new Sample(2, 2, 2, 3, 4, 5), 0.6);
    ...
    inputBuilder.Add("edge-mixed-signs",
      new Sample(-5, -3, -1, 1, 3, 5, 7, 9, 11, 13),
      new Sample(-4, -2, 0, 2, 4, 6, 8, 10, 12, 14),
      0.2);
    inputBuilder.Add("edge-asymmetric-8-30",
      new Sample(Enumerable.Range(1, 8).Select(x => (double)x).ToArray()),
      new Sample(Enumerable.Range(1, 30).Select(x => (double)x).ToArray()),
      0.3);
    ...
    inputBuilder.AddUnsorted("wide-range", new Sample(1000, 1, 100, 10, 10000, 100000, 1000000, 100, 10, 1),
      new Sample(10000, 100, 1, 1000, 10, 100000, 1000000, 10, 1, 100), 0.2);

    var controller = new AvgSpreadBoundsController(suiteName);
