...
public class CoverageSimulationTests
{
  ...
  // =========================================================================
  // CenterBoundsApprox Coverage (nominal coverage - document actual behavior)
  // =========================================================================

  [Theory]
  [InlineData(10, 0.10)]
  [InlineData(30, 0.05)]
  public void CenterBoundsApprox_Coverage_AdditiveDistribution(int n, double misrate)
  {
    var rng = new Rng($"center-approx-additive-{n}-{misrate}");
    double trueCenter = 0.0; // Additive(0,1) has center = 0
    int covered = 0;
    ...
    // Bootstrap has known undercoverage for small n (85-92% when requesting 95%)
    // Allow more tolerance: actual coverage may be 15% below nominal
    Assert.True(
      actualCoverage >= expectedCoverage - 0.20,
      $"CenterBoundsApprox coverage {actualCoverage:F3} should be >= {expectedCoverage - 0.20:F3} " +
      $"(n={n}, misrate={misrate}, expected={expectedCoverage}, bootstrap nominal)");
  }

  [Theory]
  [InlineData(10, 0.10)]
  [InlineData(30, 0.05)]
  public void CenterBoundsApprox_Coverage_UniformSymmetric(int n, double misrate)
  {
    var rng = new Rng($"center-approx-uniform-{n}-{misrate}");
    double trueCenter = 0.0; // Uniform(-1,1) has center = 0
    int covered = 0;
  ...
  [InlineData(10, 0.10)]
  [InlineData(30, 0.05)]
  public void CenterBoundsApprox_Coverage_ExponentialDistribution(int n, double misrate)
  {
    var rng = new Rng($"center-approx-exp-{n}-{misrate}");
    // For Exp(1), center (Hodges-Lehmann) != median
    // The HL pseudomedian for Exp(1) is approximately 0.798
    double trueCenter = 0.798;
    int covered = 0;
    ...
    // For asymmetric distributions, bootstrap may have more undercoverage
    // Just verify we have reasonable coverage (at least 60%)
    Assert.True(
      actualCoverage >= 0.60,
      $"CenterBoundsApprox coverage {actualCoverage:F3} for Exp(1) should be >= 0.60 " +
      $"(n={n}, misrate={misrate}, expected ~{expectedCoverage}, asymmetric distribution)");
  }

  // =========================================================================
  // Comparison tests: CenterBounds vs CenterBoundsApprox on symmetric data
  // =========================================================================

  [Theory]
  [InlineData(20, 0.10)]
  public void CenterBounds_ApproxComparison_SymmetricDistribution(int n, double misrate)
  {
    var rng = new Rng($"comparison-{n}-{misrate}");
    ...
    for (int i = 0; i < SmokeTestIterations; i++)
    {
      ...
      var exactBounds = Toolkit.CenterBounds(sample, new Probability(misrate));
      var approxBounds = Toolkit.CenterBoundsApprox(sample, new Probability(misrate), $"sim-{i}");

      if (exactBounds.Lower <= trueCenter && trueCenter <= exactBounds.Upper)
        coveredExact++;
      if (approxBounds.Lower <= trueCenter && trueCenter <= approxBounds.Upper)
        coveredApprox++;
    }

    double exactCoverage = (double)coveredExact / SmokeTestIterations;
    double approxCoverage = (double)coveredApprox / SmokeTestIterations;

    // Both should provide reasonable coverage
    Assert.True(
      exactCoverage >= 0.80,
      $"CenterBounds exact coverage {exactCoverage:F3} should be >= 0.80");
    Assert.True(
      approxCoverage >= 0.70,
      $"CenterBoundsApprox coverage {approxCoverage:F3} should be >= 0.70");
  }
