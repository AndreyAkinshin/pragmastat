% A color palette adopted for color-blind people based on https://jfly.uni-koeln.de/color/
\usepackage{hyperref}
\definecolor{linkcolor}{HTML}{D55E00}
\definecolor{citecolor}{HTML}{009E73}
\definecolor{urlcolor}{HTML}{0072B2}
\hypersetup{
    colorlinks,
    linkcolor={linkcolor},
    citecolor={citecolor},
    urlcolor={urlcolor}
}

% Adjust source code color palette
\makeatletter
\@ifundefined{DecValTok}{}{
  \renewcommand{\DecValTok}[1]{\textcolor[HTML]{009E73}{#1}}
  \renewcommand{\FloatTok}[1]{\textcolor[HTML]{009E73}{#1}}
  \renewcommand{\ConstantTok}[1]{\textcolor[HTML]{009E73}{#1}}
  \renewcommand{\ControlFlowTok}[1]{\textcolor[HTML]{0072B2}{\textbf{#1}}}
  \renewcommand{\OtherTok}[1]{\textcolor[HTML]{000000}{#1}}
  \renewcommand{\CommentTok}[1]{\textcolor[HTML]{999999}{\textit{#1}}}
  \renewcommand{\AttributeTok}[1]{\textcolor[HTML]{CC79A7}{#1}}
  \renewcommand{\FunctionTok}[1]{\textcolor[HTML]{56B4E9}{#1}}
}
\makeatother

% Misc
\usepackage{caption}
\captionsetup{width=14cm}
\usepackage[absolute]{textpos}

% Code listing wrapping
% Based on https://github.com/rstudio/rmarkdown/issues/646#issuecomment-926583252
\usepackage{fvextra}
\DefineVerbatimEnvironment{Highlighting}{Verbatim}{breaklines,commandchars=\\\{\}}

% Info block
\usepackage[framemethod=TikZ]{mdframed}
\definecolor{infocolor}{HTML}{6cbf43}
\newenvironment{info}{%
  \begin{mdframed}[
    leftmargin=\parindent,
    skipabove=\medskipamount,
    skipbelow=\medskipamount,
    linecolor=infocolor,
    linewidth=1pt,
    roundcorner=5pt
  ]
}{%
  \end{mdframed}
}

% Figure settings
% Center figures and use [H] positioning (requires float package from YAML)
\makeatletter
\def\fps@figure{H}
\makeatother
\let\origfigure\figure
\let\endorigfigure\endfigure
\renewenvironment{figure}[1][H]{%
  \origfigure[#1]
  \centering
}{%
  \endorigfigure
}
